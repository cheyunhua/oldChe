<template>
    <div>
        ...
        <el-dialog
                title="Slog Hook Log View As JSON"
    ...
    export default {
        ...
        computed: {
            wsUrl() {
                ...
                return `${config.wsBase}/ws/hook?&_t=${token}`
            }
        },
        mounted() {
        ...
        methods: {
            ...
            humanTime(ss) {
                ...
                if (diffs < 60) { // 一分钟以内
                    return `${Math.floor(diffs % 60)}s ago`;
                } else if (diffs < 3600) { // 一小时前之内
                    return `${Math.floor(diffs / 60)}m ago`;
                } else if (diffs < 3600 * 24) { // 一小时前之内
